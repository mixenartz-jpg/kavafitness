...
import BodyModel from '../3d/BodyModel'
...
// Egzersiz adından kas gruplarını tahmin et
const MUSCLE_MAP = [
...
`
  ...
  const send = async (text) => {
    ...
    if (reply) {
      ...
      // Try to detect muscle context from user message
  ...
  return (
    <div className="page animate-fade" style={{ maxWidth: 700, paddingBottom: 100 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        <div style={{ width: 46, height: 46, borderRadius: 14, background: 'linear-gradient(135deg,rgba(232,255,71,.15),rgba(71,200,255,.1))', border: '1px solid rgba(232,255,71,.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, flexShrink: 0 }}>
          🏋️
        </div>
        <div>
      ...
      <div className="card" style={{ overflow: 'hidden', marginBottom: 12, padding: 0 }}>
        <BodyModel activeGroups={activeGroups} height={260} />
        {activeGroups.length > 0 && (
          <div style={{ padding: '10px 16px', display: 'flex', flexWrap: 'wrap', gap: 8, borderTop: '1px solid var(--border)' }}>
            {MUSCLE_LEGEND.filter(m => activeGroups.includes(m.key)).map(m => (
              <span key={m.key} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: 'var(--text)', fontFamily: 'Space Mono,monospace' }}>
        ...
        {activeGroups.length === 0 && (
          <div style={{ padding: '10px 16px', fontFamily: 'Space Mono,monospace', fontSize: 10, color: 'var(--text-muted)', borderTop: '1px solid var(--border)' }}>
            Antrenman yazınca çalışan kaslar renklenecek ✨
          </div>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
        {QUICK_PROMPTS.map(q => (
          <button
            ...
            onClick={() => send(q)}
            disabled={loading}
            style={{ padding: '6px 13px', borderRadius: 20, border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--accent)', fontFamily: 'Space Mono,monospace', fontSize: 10, cursor: 'pointer', opacity: loading ? 0.5 : 1, transition: 'all .15s' }}
          >
            {q}
          </button>
        ))}
        ...
        <div style={{ padding: '14px 14px 0', maxHeight: 380, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 12, scrollbarWidth: 'thin' }}>
          {messages.map((m, i) => (
            <div key={i} style={{ display: 'flex', gap: 8, flexDirection: m.role === 'user' ? 'row-reverse' : 'row', alignItems: 'flex-start' }}>
              <div style={{ width: 28, height: 28, borderRadius: '50%', flexShrink: 0, background: m.role === 'user' ? 'var(--accent)' : 'rgba(232,255,71,.15)', border: m.role === 'user' ? 'none' : '1px solid rgba(232,255,71,.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: m.role === 'user' ? '#0a0a0a' : 'var(--accent)' }}>
                {m.role === 'user' ? '👤' : '🏋️'}
              </div>
              <div style={{ maxWidth: '80%', background: m.role === 'user' ? 'rgba(232,255,71,.1)' : 'var(--surface2)', border: m.role === 'user' ? '1px solid rgba(232,255,71,.2)' : '1px solid var(--border)', borderRadius: m.role === 'user' ? '14px 4px 14px 14px' : '4px 14px 14px 14px', padding: '10px 14px' }}>
      ...
      <style>{`@keyframes ptBounce{0%,80%,100%{transform:scale(.8);opacity:.5}40%{transform:scale(1.2);opacity:1}}`}</style>
    </div>
  )
}
